...
在实际部署使用时，如果您发现按下快捷键 `Control + Option + G` 后**并未开启语音录音，而是触发了窗口大小调整/右侧缩放**，这是因为您的 macOS 上安装了如 **Magnet** 或 **Rectangle** 等第三方窗口管理工具。

### 1. 冲突原因
* **Magnet 默认配置**：Magnet 的默认快捷键列表里，`Control + Option + G` 被硬编码映射为了 **“右侧三分之一窗口缩放（Right Third）”**。因为窗口管理器捕获键盘的优先级较高，直接拦截了按键信号，导致快捷指令无法响应。

### 2. 解决方案
您可以选择以下任一方式完美解决：
* **解决方案 A（推荐，保留 G 键意指 Gemini）**：
  点击 macOS 顶部菜单栏的 Magnet 图标进入「偏好设置 -> 快捷键」，找到 **“右侧三分之一（Right Third）”** 这一项，点击它并按下 `Delete` 或 `Esc` 键**禁用**该快捷键，让出 `Control + Option + G` 的系统级绑定。
* **解决方案 B（更换语音听写快捷键）**：
  如果您不想修改 Magnet 的默认值，可以打开 macOS **「快捷指令」** App，右键点击 **“Gemini 语音听写”**，选择 **「快速操作」 ➡️ 「添加键盘快捷键」**，将其修改为一个绝对不冲突的快捷键，例如 **`Control + Option + S`**（S 意为 **S**peech 或 **S**TT），同样极为丝滑。
