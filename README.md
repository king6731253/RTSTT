...
4. 在终端中运行以下命令，强制结束当前的常驻进程：
   ...
   ```
5. **无需手动重启**：当您下一次按下听写快捷键 `Control+Command+G` 时，控制脚本会检测到进程不在，并**自动静默拉起载入了您最新规则的守护进程**。
...
* **快捷指令（Apple Shortcut）绑定**：全局快捷键 `Control+Command+G` 被映射为在后台静默运行 shell 命令 `/Users/yuanjin/localbin/dictate_toggle.sh`。
* **首次运行标志**：权限自检成功后会在家目录下生成空文件 `~/.gemini_dictation_checked`，以此跳过后续 of the 重复权限检测，实现微秒级零延迟。

---


## ⚠️ 6. 快捷键冲突排除说明（针对 Magnet / Rectangle 等窗口管理器用户）

为了完美解决此冲突，当前系统已将全局听写快捷键更换为：**`Control + Command + G`**。

### 1. 冲突排查（历史背景）
* **Magnet 默认冲突**：在之前的配置中，`Control + Option + G` 与 Magnet 的 **“右侧三分之一窗口缩放（Right Third）”** 存在默认快捷键冲突。由于窗口管理器捕获优先级高，导致快捷指令无响应。

### 2. 当前绑定与更换方式
* **当前生效快捷键**：**`Control + Command + G`**
* **更换语音听写快捷键**：
  如果您未来需要再次修改，可以打开 macOS **「快捷指令」** App，右键点击 **“Gemini 语音听写”**，选择 **「快速操作」 ➡️ 「添加键盘快捷键」**，将其修改为您心仪的组合键即可。
...
```text
我需要你在 macOS 上为我开发一个名为 RTSTT (Real-Time Speech to Text) 的全局语音听写系统。系统由快捷键 Control+Command+G 触发，按一次开始录音，再按一次结束录音，并将听写的文字自动粘贴到当前活跃的输入框中。为了达到极致的“秒级响应”体验，整个项目必须在底层进行如下深度速度优化：
